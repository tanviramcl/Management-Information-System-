...
namespace AMCLBL
{
    public class DividendDAO
    {
        ...
        public DataTable dtFY()
        {
            ...
            DataTable dtList = commonGatewayObj.Select("SELECT DISTINCT FY FROM AMCL_DIVIDEND.DIVIDEND_PARA ORDER BY FY DESC");
        ...
        public DataTable dtFY(int f_code)
        {

            DataTable dtList = commonGatewayObj.Select("SELECT  FY, DIVI_NO FROM DIVIDEND_PARA WHERE FUND_CODE=" + f_code + " ORDER BY FY DESC");
            DataTable dtDropDown = new DataTable();
            dtDropDown.Columns.Add("FY", typeof(string));
        ...
        public DataTable dtRecordDateFYWise(string FY)
        {
            ...
            DataTable dtList = commonGatewayObj.Select("SELECT DISTINCT TO_CHAR(RECORD_DATE,'DD-MON-YYYY') AS RECORD_DATE FROM AMCL_DIVIDEND.DIVIDEND_PARA WHERE FY='" + FY + "' ORDER BY RECORD_DATE DESC");
            DataTable dtDropDown = new DataTable();
        ...
        public DataTable Get_FundInfo_FROM_DIVIPARABYFUND(string FY,string Record_date,string F_cd)
        {

            DataTable dtCloseendFund = new DataTable();
            StringBuilder sbMst = new StringBuilder();
            sbMst.Append(" select a.*,b.F_NAME,b.F_type from (select * from AMCL_DIVIDEND.DIVIDEND_PARA where  RECORD_DATE='"+Record_date+"' and FY='"+FY+ "' order by fund_code) a inner join INVEST.FUND b on a.fund_code=b.f_cd where f_cd="+F_cd+" ") ;
            dtCloseendFund = commonGatewayObj.Select(sbMst.ToString());


            return dtCloseendFund;
        }
        public DataTable Get_FundInfo_FROM_DIVIPARA(string FY, string Record_date)
        {

            DataTable dtCloseendFund = new DataTable();
            StringBuilder sbMst = new StringBuilder();
            sbMst.Append(" select a.*,b.F_NAME,b.F_type from (select * from AMCL_DIVIDEND.DIVIDEND_PARA where  RECORD_DATE='" + Record_date + "' and FY='" + FY + "' order by fund_code) a inner join INVEST.FUND b on a.fund_code=b.f_cd ");
            dtCloseendFund = commonGatewayObj.Select(sbMst.ToString());
        ...
        public DataTable Get_WarrentInfo_BY_BO(string FY, string Record_date ,string f_cd,string BO )
        {

            DataTable dtDividendInfoByBO = new DataTable();
            StringBuilder sbMst = new StringBuilder();
            sbMst.Append("  SELECT INVEST.FUND.*, DIVIDEND.*, DIVIDEND_PARA.* FROM AMCL_DIVIDEND.DIVIDEND, INVEST.FUND, ");
            sbMst.Append(" AMCL_DIVIDEND.DIVIDEND_PARA WHERE DIVIDEND.FUND_CODE = INVEST.FUND.F_CD  AND ");
            sbMst.Append("   DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND DIVIDEND.DIVI_NO = DIVIDEND_PARA.DIVI_NO AND ");
            sbMst.Append(" DIVIDEND.FY = DIVIDEND_PARA.FY AND  DIVIDEND.RECORD_DATE = DIVIDEND_PARA.RECORD_DATE ");
            sbMst.Append(" AND(DIVIDEND.FY = '"+FY+"') AND DIVIDEND.RECORD_DATE = '"+ Record_date+"'   and f_cd = "+ f_cd+"  and ");
            sbMst.Append(" EMAIL IS NOT NULL AND EMAIL LIKE'%@%' AND REGEXP_LIKE(EMAIL, '^[A-Za-z]+[A-Za-z0-9.]+@[A-Za-z0-9.-]+.[A-Za-z]{2,4}$') AND BO='"+BO+"'");
            dtDividendInfoByBO = commonGatewayObj.Select(sbMst.ToString());


            return dtDividendInfoByBO;
        }
        public DataTable dtBEFTNResendDate(string FY, int fundCode, string RecordDate)
        {
            string sqlQuery = " SELECT DISTINCT TO_CHAR(BEFTN_RESEND.BEFTN_SEND_DATE, 'DD-MON-YYYY') AS BEFTN_DATE FROM   BEFTN_RESEND INNER JOIN  DIVIDEND ON BEFTN_RESEND.DIVIDEND_ID = DIVIDEND.ID ";
            sqlQuery = sqlQuery + " WHERE (BEFTN_RESEND.BEFTN_SEND_DATE IS NOT NULL) AND (DIVIDEND.FY = '" + FY + "') AND (DIVIDEND.FUND_CODE = " + fundCode + ") AND (DIVIDEND.RECORD_DATE = '" + RecordDate + "')";
            sqlQuery = sqlQuery + " ORDER BY BEFTN_DATE DESC ";

            DataTable dtList = commonGatewayObj.Select(sqlQuery.ToString());
            DataTable dtDropDown = new DataTable();
            dtDropDown.Columns.Add("BEFTN_DATE", typeof(string));
            dtDropDown.Columns.Add("BEFTN_DATE_VALUE", typeof(string));

            DataRow drdtDropDown = dtDropDown.NewRow();

            drdtDropDown["BEFTN_DATE"] = "--Select Record Date--- ";
            drdtDropDown["BEFTN_DATE_VALUE"] = "0";
            dtDropDown.Rows.Add(drdtDropDown);
            for (int loop = 0; loop < dtList.Rows.Count; loop++)
            {
                drdtDropDown = dtDropDown.NewRow();
                drdtDropDown["BEFTN_DATE"] = dtList.Rows[loop]["BEFTN_DATE"].ToString();
                drdtDropDown["BEFTN_DATE_VALUE"] = dtList.Rows[loop]["BEFTN_DATE"].ToString();
                dtDropDown.Rows.Add(drdtDropDown);
            }
            return dtDropDown;
        }
        ...
        public DataTable dtGetCDBLDataSingleFile(int fundCode, string ficalYear, string recordDate, string filter)
        {
            ...
            sb.Append(" CITY, COUNTRY, POST_CODE, PHONE1, PHONE2, RESIDENCY, BANK, BRANCH, BANK_ACC, ROUTING_NO, BANK_ID, IS_BEFTN, ETIN, IS_VALID_ETIN ,DP_ID ");
        ...
        public DataTable dtGetFolioWindingUpData(int fundCode)
        {
            ...
            //sb.Append("SELECT * FROM WINDING_UP_DATA WHERE (VOTTER_NO IS NULL) AND (VALID IS NULL) AND (ALLOT_NO IS NOT NULL) AND FUND_CODE = " + fundCode + " ORDER BY TO_NUMBER(ALLOT_NO)");
        ...
        public DataTable GetLetterOfEntitlementofUnitsData(int fundCode, string filter)
        {
            ...
            sb.Append(" WINDING_UP_DATA.FRACTION_AMT,WINDING_UP_DATA.VOTTER_NO ");
        ...
        public DataTable GetDividendWarrantPrintData(int fundCode, int diviNo, string filter)
        {
            ...
            sb.Append(" DIVIDEND.ADDRESS1, DIVIDEND.ADDRESS2, DIVIDEND.PHONE1,DIVIDEND.PHONE2,");
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" SELECT     BO, NAME1, BANK, BRANCH, BANK_ACC ");
            sb.Append(" FROM         CDBL_DATA ");
        ...
        public string getOmnibusBO(string tableName, int fundCode, string fisclYear, int diviNo)
        {
            string mutualFundOrOmnibusBO = "";
            DataTable dtMutualFundOrOmnibusBO = commonGatewayObj.Select("SELECT BO FROM " + tableName.ToString() + " WHERE VALID IS NULL AND BO_CATAGORY='OMNIBUS' AND FUND_CODE = " + fundCode + " AND FY = '" + fisclYear + "' AND DIVI_NO = " + diviNo + "");
            if (dtMutualFundOrOmnibusBO.Rows.Count > 0)
            {
                for (int loop = 0; loop < dtMutualFundOrOmnibusBO.Rows.Count; loop++)
                {
        ...
        public DataTable GetWarrantInfoTax(string dividendID)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append(" SELECT    INVEST.FUND.F_CD, INVEST.FUND.F_NAME, DIVIDEND_PARA.FY, TO_CHAR(DIVIDEND_PARA.RECORD_DATE, 'DD-MON-YYYY') AS RECORD_DATE,  ");
            sb.Append(" TO_CHAR(DIVIDEND_PARA.AGM_DATE, 'DD-MON-YYYY') AS AGM_DATE, TO_CHAR(DIVIDEND_PARA.ISSUE_DATE, 'DD-MON-YYYY') AS ISSUE_DATE, ");
            sb.Append(" LTRIM(TO_CHAR(DIVIDEND_PARA.FACE_VALUE * DIVIDEND_PARA.DIVI_RATE / 100, '99999999999.99'), ' ') AS DIVI_RATE_PER_UNIT, ");
            sb.Append(" DIVIDEND_PARA.TAX_RATE_INDV || '%' AS TAX_RATE_INDV, DIVIDEND_PARA.TAX_RATE_ORG || '%' AS TAX_RATE_ORG, ");
            sb.Append(" DIVIDEND_PARA.BANK_NAME, DIVIDEND_PARA.BANK_ACC_NO, DIVIDEND_PARA.BANK_ACC_NO_MICR || 'C' AS BANK_ACC_NO_MICR, ");
            ...
            sb.Append(" DIVIDEND.bank as bank_name_unit_holder, DIVIDEND.branch as bank_branch_unit_holder, DIVIDEND.bank_acc as bank_acc_unit_holder, ");
            sb.Append(" 'Taka ' || trim(DIVIDEND.NET_DIVIDEND_IN_WORDS) || ' Only' AS DIVI_WORD ");
            sb.Append(" FROM         AMCL_DIVIDEND.DIVIDEND INNER JOIN ");
            sb.Append(" AMCL_DIVIDEND.DIVIDEND_PARA ON DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND DIVIDEND.DIVI_NO = DIVIDEND_PARA.DIVI_NO INNER JOIN ");
            sb.Append(" INVEST.FUND ON DIVIDEND_PARA.FUND_CODE = INVEST.FUND.F_CD ");
            sb.Append(" WHERE (DIVIDEND.VALID IS NULL) AND (DIVIDEND.WAR_NO IS NOT NULL) AND  DIVIDEND.ID IN(" + dividendID + ") ");
            sb.Append(" ORDER BY DIVIDEND.WAR_NO ");
        ...
        public DataTable getdtWarrantInfo()
        {
            ...
            dtWarrantInfoTable.Columns.Add("BEFTN_RESEND_ID", typeof(string));
            dtWarrantInfoTable.Columns.Add("FUND_NAME", typeof(string));
            dtWarrantInfoTable.Columns.Add("WAR_NO", typeof(string));
            dtWarrantInfoTable.Columns.Add("FY", typeof(string));
            ...
            dtWarrantInfoTable.Columns.Add("DUPLICATE_ISSUE_DATE", typeof(string));
        ...
        public DataTable getdtRecordDateData()
        {
            ...
            dtWarrantInfoTable.Columns.Add("ETIN", typeof(string));
            dtWarrantInfoTable.Columns.Add("IS_VALID_ETIN", typeof(string));
            dtWarrantInfoTable.Columns.Add("DP_ID", typeof(string));
            dtWarrantInfoTable.Columns.Add("DP_UPDATE_BY", typeof(string));
            dtWarrantInfoTable.Columns.Add("DP_CHECK", typeof(int));
        ...
        public DataTable getMarchantBankBOInfo()
        {
            ...
            dtMarchantBankBo.Columns.Add("FUND", typeof(string));
            ...
            dtMarchantBankBo.Columns.Add("IS_VALID_ETIN", typeof(string));
            dtMarchantBankBo.Columns.Add("ADRESS1", typeof(string));
        ...
        {
            DataTable dtOmnibusCompanyList = commonGatewayObj.Select("SELECT     BO, NAME1 AS NAME,CITY FROM  CDBL_DATA WHERE " + filter + "   AND (BO_CATAGORY = 'OMNIBUS')AND (FUND_CODE = " + fundCode + ")AND (DIVI_NO = " + diviNo + ") AND  (FY = '" + fiscalYear + "') ORDER BY NAME");
            DataTable dtOmnibusCompanyListDropDown = new DataTable();
            dtOmnibusCompanyListDropDown.Columns.Add("BO", typeof(string));
            dtOmnibusCompanyListDropDown.Columns.Add("NAME", typeof(string));
        ...
        public DataTable dtListOfMarchantBankForNewEntry()
        {
            DataTable dtMarchantBankList = commonGatewayObj.Select("SELECT     DP_ID, DP_NAME || ' (' || DP_ID || ')' AS NAME FROM MARCHANTBANK  WHERE DP_ID NOT IN (SELECT DISTINCT DP_ID FROM MARCHANTBANK_DIVIDEND_BO_LIST WHERE FY='2014-2015' )ORDER BY DP_NAME");
            DataTable dtMarchantBankListDropDown = new DataTable();
            dtMarchantBankListDropDown.Columns.Add("DP_ID", typeof(string));
            dtMarchantBankListDropDown.Columns.Add("NAME", typeof(string));
            ...
            drMarchantBankListDropDown["NAME"] = "--Select Marchant Bank Company--- ";
            drMarchantBankListDropDown["DP_ID"] = "0";
            dtMarchantBankListDropDown.Rows.Add(drMarchantBankListDropDown);
            for (int loop = 0; loop < dtMarchantBankList.Rows.Count; loop++)
            {
        ...
        {
            DataTable dtOmnibusCompanyName = commonGatewayObj.Select("SELECT NAME1 FROM CDBL_DATA WHERE " + filter);
            string name = dtOmnibusCompanyName.Rows[0]["NAME1"].Equals(DBNull.Value) ? "" : dtOmnibusCompanyName.Rows[0]["NAME1"].ToString().ToUpper();
            return name;
        }
        ...
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT  MARCHANTBANK_DIVIDEND.DP_ID, MARCHANTBANK_DIVIDEND.FUND_CODE, MARCHANTBANK_DIVIDEND.FY, MARCHANTBANK_DIVIDEND.DIVI_NO, ");
            sb.Append(" CDBL_DP_LIST.DP_NAME AS NAME1, CDBL_DP_LIST.ADDRESS AS ADDRESS1, CDBL_DP_LIST.BANK_ACC_NO AS BANK_ACC, CDBL_DP_LIST.BANK_NAME AS BANK, CDBL_DP_LIST.BANK_BRANCH AS BRANCH, ");
            sb.Append(" CDBL_DP_LIST.ROUTING_NO, MARCHANTBANK_DIVIDEND.GROSS_DIVIDEND, MARCHANTBANK_DIVIDEND.DIDUCT,  MARCHANTBANK_DIVIDEND.NET_DIVIDEND ");
            sb.Append(" ,MARCHANTBANK_DIVIDEND.BALANCE FROM  CDBL_DP_LIST INNER JOIN  MARCHANTBANK_DIVIDEND ON CDBL_DP_LIST.DP_ID = MARCHANTBANK_DIVIDEND.DP_ID ");
            sb.Append(" WHERE   (MARCHANTBANK_DIVIDEND.IS_PROCESS IS NULL) AND(MARCHANTBANK_DIVIDEND.FY = '" + fy + "') AND (MARCHANTBANK_DIVIDEND.FUND_CODE = " + fundCode + ") AND (MARCHANTBANK_DIVIDEND.DIVI_NO = " + diviNo + ")");
        ...
        public DataTable GetdtCdblData(int fundCode, int diviNo, string bo)
        {
            ...
            sb.Append(" FROM         CDBL_DATA  ");
            sb.Append(" WHERE   ROUTING_NO IS NULL AND  (VALID IS NULL) AND (FUND_CODE = " + fundCode + ") AND (DIVI_NO = " + diviNo + ") AND (BO = '" + bo + "') ");
        ...
        public DataTable getdtDPListforDropdownlist()
        {
            DataTable dtMarchantBankList = commonGatewayObj.Select("SELECT     DP_ID, DP_NAME || ' (' || DP_ID || ')' AS NAME FROM CDBL_DP_LIST  ORDER BY DP_NAME");
            DataTable dtMarchantBankListDropDown = new DataTable();
            dtMarchantBankListDropDown.Columns.Add("DP_ID", typeof(string));
            dtMarchantBankListDropDown.Columns.Add("NAME", typeof(string));
            ...
            drMarchantBankListDropDown["NAME"] = "--Select Dp Name--- ";
            drMarchantBankListDropDown["DP_ID"] = "0";
            dtMarchantBankListDropDown.Rows.Add(drMarchantBankListDropDown);
            for (int loop = 0; loop < dtMarchantBankList.Rows.Count; loop++)
            {
        ...
        public DataTable getdtDPListforDropdownlist(string filterDPType)
        {
            DataTable dtMarchantBankList = commonGatewayObj.Select("SELECT     DP_ID, DP_NAME || ' (' || DP_ID || ')' AS NAME FROM CDBL_DP_LIST WHERE DP_TYPE='" + filterDPType + "' ORDER BY DP_NAME");
            DataTable dtMarchantBankListDropDown = new DataTable();
            dtMarchantBankListDropDown.Columns.Add("DP_ID", typeof(string));
            dtMarchantBankListDropDown.Columns.Add("NAME", typeof(string));
        ...
        public DataTable dtGetBEFTNResendData(int fundCode, int diviNo, string fy, string recordDate, string filter)
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT  DIVIDEND.BO, DIVIDEND.RECORD_DATE, DIVIDEND.FY, DIVIDEND.DIVI_NO, DIVIDEND.FUND_CODE, DIVIDEND.ALLOT_NO, DIVIDEND.FOLIO_NO, ");
            sbQuery.Append(" DIVIDEND.WAR_NO, DIVIDEND.NAME1, DIVIDEND.BANK, DIVIDEND.BRANCH, DIVIDEND.BANK_ACC, DIVIDEND.NET_DIVIDEND, DIVIDEND.ROUTING_NO, ");
            sbQuery.Append(" DIVIDEND_PARA.BANK_NAME, DIVIDEND_PARA.BANK_ADDRS1, DIVIDEND_PARA.BANK_ADDRS2, DIVIDEND_PARA.BANK_ROUTING_NO,");
            sbQuery.Append(" DIVIDEND_PARA.BANK_TRANSACTION_NO, BEFTN_RESEND.BEFTN_SEND_DATE, DIVIDEND_PARA.BANK_ACC_NO_MICR, DIVIDEND.DP_ID, DIVIDEND.IS_BEFTN, ");
            sbQuery.Append(" DIVIDEND.IS_BEFTN_SUCCESS, DIVIDEND.BEFTN_RETURN_DATE FROM  DIVIDEND INNER JOIN   BEFTN_RESEND ON DIVIDEND.ID = BEFTN_RESEND.DIVIDEND_ID INNER JOIN ");
            sbQuery.Append(" DIVIDEND_PARA ON DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND DIVIDEND.DIVI_NO = DIVIDEND_PARA.DIVI_NO AND ");
            sbQuery.Append(" DIVIDEND.FY = DIVIDEND_PARA.FY AND DIVIDEND.RECORD_DATE = DIVIDEND_PARA.RECORD_DATE ");
            sbQuery.Append(" WHERE  (BEFTN_RESEND.VALID IS NULL) AND " + filter.ToString() + " AND (DIVIDEND.DIVI_NO = " + diviNo + ") ");
            sbQuery.Append(" AND (DIVIDEND.FUND_CODE = " + fundCode + ") AND (DIVIDEND.FY = '" + fy + "') AND   (DIVIDEND.RECORD_DATE = '" + recordDate + "')");
            sbQuery.Append(" ORDER BY DIVIDEND.WAR_NO ");

            DataTable dtBeftnResendInfo = commonGatewayObj.Select(sbQuery.ToString());
            return dtBeftnResendInfo;

        }
        public DataTable dtGetBEFTNResendData(string filter)
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT INVEST.FUND.*, DIVIDEND.*, DIVIDEND_PARA.* FROM DIVIDEND, INVEST.FUND, DIVIDEND_PARA WHERE DIVIDEND.FUND_CODE = INVEST.FUND.F_CD ");
            sbQuery.Append(" AND DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND DIVIDEND.DIVI_NO = DIVIDEND_PARA.DIVI_NO AND DIVIDEND.FY = DIVIDEND_PARA.FY AND  DIVIDEND.RECORD_DATE = DIVIDEND_PARA.RECORD_DATE ");
            sbQuery.Append(" AND DIVIDEND.BEFTN_RESENT_REQ_DATE IS NOT NULL ");
            sbQuery.Append(filter);
            DataTable dtBeftnResendInfo = commonGatewayObj.Select(sbQuery.ToString());
            return dtBeftnResendInfo;

        }
        public DataTable dtBEFTNDateList()
        {
            DataTable dtList = commonGatewayObj.Select(" SELECT DISTINCT TO_CHAR(BEFTN_RESENT_DATE, 'DD-MON-YYYY') AS NAME,  TO_CHAR(BEFTN_RESENT_DATE, 'DD-MON-YYYY') AS ID FROM DIVIDEND WHERE BEFTN_RESENT_REQ_DATE IS NOT NULL AND BEFTN_RESENT_AUDIT_BY IS NOT NULL AND BEFTN_RESENT_PROCESS_BY IS NOT NULL AND BEFTN_RESENT_TRACKING_NO IS NULL ORDER BY 1 ");
        ...
        public DataTable dtBEFTNFYWiseFundName(string filter)
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT  DISTINCT  DIVIDEND.FUND_CODE||'-'|| DIVIDEND.DIVI_NO AS ID,   INVEST.FUND.F_NAME||' ['|| DIVIDEND.FY||'] ' AS NAME");
            sbQuery.Append(" FROM  INVEST.FUND, DIVIDEND_PARA, DIVIDEND WHERE INVEST.FUND.F_CD = DIVIDEND_PARA.FUND_CODE AND DIVIDEND_PARA.FUND_CODE = DIVIDEND.FUND_CODE");
            sbQuery.Append(" AND DIVIDEND_PARA.DIVI_NO = DIVIDEND.DIVI_NO AND DIVIDEND_PARA.FY = DIVIDEND.FY AND  DIVIDEND_PARA.RECORD_DATE = DIVIDEND.RECORD_DATE ");
            sbQuery.Append(" AND  DIVIDEND.BEFTN_RESENT_REQ_DATE IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_PROCESS_BY IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_AUDIT_BY IS NOT NULL ");
            sbQuery.Append(filter);
            ...
            drListDropDown["ID"] = "0";
            dtListDropDown.Rows.Add(drListDropDown);
            for (int loop = 0; loop < dtBeftnResendInfo.Rows.Count; loop++)
            {
                drListDropDown = dtListDropDown.NewRow();
                drListDropDown["NAME"] = dtBeftnResendInfo.Rows[loop]["NAME"].ToString();
                drListDropDown["ID"] = dtBeftnResendInfo.Rows[loop]["ID"].ToString();
                dtListDropDown.Rows.Add(drListDropDown);
            }
            return dtListDropDown;
        }
        ...
        {
            StringBuilder sbQuery = new StringBuilder();
            sbQuery.Append(" SELECT INVEST.FUND.F_NAME,  INVEST.FUND.F_NAME||' DIVIDEND ACCOUNT  '|| DIVIDEND_PARA.FY AS DIVI_ACC_NAME,DECODE( DIVIDEND_PARA.FUND_CODE,14,'MSND','SND') AS ACC_TYPE, ");
            sbQuery.Append(" DIVIDEND_PARA.FY, DIVIDEND_PARA.BANK_ACC_NO, DIVIDEND_PARA.BANK_ROUTING_NO, UNIT.BANK_NAME.BANK_NAME,  DIVIDEND.BEFTN_RESENT_DATE,");
            sbQuery.Append(" UNIT.BANK_BRANCH.BRANCH_NAME, UNIT.BANK_BRANCH.BRANCH_ADDRS1, UNIT.BANK_BRANCH.BRANCH_ADDRS2, UNIT.BANK_BRANCH.BRANCH_POST_CODE,DIVIDEND.BEFTN_RESENT_TRACKING_NO, ");
            sbQuery.Append(" DIVIDEND.BO, DIVIDEND.WAR_NO, DIVIDEND.NAME1, DIVIDEND.BANK, DIVIDEND.BRANCH,  DIVIDEND.BANK_ACC, DIVIDEND.ROUTING_NO,  DIVIDEND.NET_DIVIDEND ");
            sbQuery.Append(" FROM  UNIT.BANK_BRANCH, UNIT.BANK_NAME, DIVIDEND, DIVIDEND_PARA, INVEST.FUND WHERE  UNIT.BANK_BRANCH.BANK_CODE = UNIT.BANK_NAME.BANK_CODE ");
            sbQuery.Append(" AND UNIT.BANK_BRANCH.ROUTING_NO = DIVIDEND_PARA.BANK_ROUTING_NO AND DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND ");
            sbQuery.Append(" DIVIDEND_PARA.FUND_CODE = INVEST.FUND.F_CD AND DIVIDEND_PARA.FY= DIVIDEND.FY AND DIVIDEND_PARA.DIVI_NO= DIVIDEND.DIVI_NO ");
            sbQuery.Append(" AND  DIVIDEND.BEFTN_RESENT_REQ_DATE IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_PROCESS_BY IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_AUDIT_BY IS NOT NULL ");
        ...
        public decimal getBEFTNAdviceTotalAmount(string filter)
        {
            ...
            sbQuery.Append(" SELECT SUM(NET_DIVIDEND) AS TOTAL_AMOUNT FROM DIVIDEND WHERE DIVIDEND.BEFTN_RESENT_REQ_DATE IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_PROCESS_BY IS NOT NULL AND  DIVIDEND.BEFTN_RESENT_AUDIT_BY IS NOT NULL ");
        ...
        public long getMaxBEFTNTrackingNo()
        {
            DataTable dtMaxID = commonGatewayObj.Select("SELECT MAX(NVL(BEFTN_RESENT_TRACKING_NO,0)) AS BEFTN_RESENT_TRACKING_NO FROM DIVIDEND");
            long MaxID = dtMaxID.Rows[0]["BEFTN_RESENT_TRACKING_NO"].Equals(DBNull.Value) ? 0 : Convert.ToInt32(dtMaxID.Rows[0]["BEFTN_RESENT_TRACKING_NO"].ToString());
            return MaxID;
        }



        //Dividend Recosiliation 
        public DataTable getDividendPara(int fundCode, int diviNo, string fy, string filter)
        {
            DataTable dtDividendPara = commonGatewayObj.Select("SELECT * FROM DIVIDEND_PARA WHERE FUND_CODE=" + fundCode + " AND FY = '" + fy + "' AND DIVI_NO=" + diviNo + " " + filter);
            return dtDividendPara;
        }
        public DataTable getDividendProcessInfo(int fundCode, int diviNo, string fy, string filter)
        {
            DataTable dtDividendPara = commonGatewayObj.Select("SELECT NVL(COUNT(*),0) AS NO_HOLDER, NVL(SUM(BALANCE),0) AS NO_UNITS, NVL(SUM(GROSS_DIVIDEND),0) AS GROSS, NVL(SUM(DIDUCT),0) AS TAX_AMOUNT, NVL(SUM(NET_DIVIDEND),0) AS NET_AMOUNT FROM DIVIDEND WHERE FUND_CODE=" + fundCode + " AND FY = '" + fy + "' AND DIVI_NO=" + diviNo + " " + filter);
            return dtDividendPara;
        }
        // public DataTable getDividendPaidInfoEFT(int fundCode, int diviNo, string fy, string filter)
        //{
        //    StringBuilder sbQuery = new StringBuilder();
        //    sbQuery.Append(" SELECT NVL(SUM(NO_HOLDER),0) AS NO_HOLDER, NVL(SUM(NO_UNITS),0) AS NO_UNITS, NVL(SUM(GROSS),0) AS GROSS, NVL(SUM(TAX_AMOUNT),0) AS TAX_AMOUNT, NVL(SUM(NET_AMOUNT),0) AS NET_AMOUNT ");
        //    sbQuery.Append(" FROM (SELECT COUNT(*) AS NO_HOLDER, SUM(BALANCE) AS NO_UNITS, SUM(GROSS_DIVIDEND) AS GROSS, SUM(DIDUCT) AS TAX_AMOUNT, SUM(NET_DIVIDEND) AS NET_AMOUNT");
        //    sbQuery.Append(" FROM DIVIDEND WHERE (FY = '" + fy + "') AND (FUND_CODE = " + fundCode + ") AND (IS_BEFTN = 'YES') AND (BEFTN_RETURN_DATE IS NULL) UNION ALL ");
        //    sbQuery.Append(" SELECT COUNT(*) AS NO_HOLDER, SUM(BALANCE) AS NO_UNITS, SUM(GROSS_DIVIDEND) AS GROSS, SUM(DIDUCT) AS TAX_AMOUNT, SUM(NET_DIVIDEND) AS NET_AMOUNT ");
        //    sbQuery.Append(" FROM DIVIDEND DIVIDEND_1 WHERE (FY = '" + fy + "') AND (FUND_CODE = " + fundCode + ") AND (IS_BEFTN = 'YES') AND (BEFTN_RETURN_DATE IS NOT NULL) AND (IS_PAID IS NOT NULL)) derivedtbl_1 WHERE 1=1  ");
        //    sbQuery.Append(filter.ToString());
        //    DataTable dtDivideninfo = commonGatewayObj.Select(sbQuery.ToString());


        //    return dtDivideninfo;
        //}
        public DataTable getDividendPayDetailsInfo(int fundCode, int diviNo, string fy, string filter)
        {
            DataTable dtDividendPara = commonGatewayObj.Select("SELECT * FROM DIVIDEND WHERE FUND_CODE=" + fundCode + " AND FY = '" + fy + "' AND DIVI_NO=" + diviNo + " " + filter + " ORDER BY ID ");
            return dtDividendPara;
        }
        ...
        public string getWarrantStatus(DataTable dtSource, int looper)
        {
            ...
            if (!(dtSource.Rows[looper]["IS_PAID"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
            ...
            if (!(dtSource.Rows[looper]["BANK_PAYMENT_DATE"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
                    WarDetails = "Bank Paid Date: " + Convert.ToDateTime(dtSource.Rows[looper]["BANK_PAYMENT_DATE"]).ToString("dd-MMM-yyyy");
                }
                else
                {
            ...
            if (!(dtSource.Rows[looper]["BEFTN_CREDIT_DATE"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
                    WarDetails = " BEFTN CREDIT Date: " + Convert.ToDateTime(dtSource.Rows[looper]["BEFTN_CREDIT_DATE"]).ToString("dd-MMM-yyyy");
                }
                else
                {
                    WarDetails = WarDetails + " ; BEFTN CREDIT Date: " + Convert.ToDateTime(dtSource.Rows[looper]["BEFTN_CREDIT_DATE"]).ToString("dd-MMM-yyyy");
                }
            }
            ...
            if (((dtSource.Rows[looper]["IS_BEFTN"].ToString() == "YES")) || !(dtSource.Rows[looper]["ONLINE_CREDIT_DATE"].Equals(DBNull.Value)))
            {
                string routingNo = dtSource.Rows[0]["ROUTING_NO"].Equals(DBNull.Value) ? "" : dtSource.Rows[0]["ROUTING_NO"].ToString();

                if (WarDetails == "")
                {
                    WarDetails = " ACC:" + dtSource.Rows[0]["BANK_ACC"].ToString() + ", " + dtSource.Rows[0]["BANK"].ToString() + " ," + dtSource.Rows[0]["BRANCH"].ToString() + "[" + routingNo + "]";
                }
                else
                {
                    WarDetails = WarDetails + " ; ACC:" + dtSource.Rows[0]["BANK_ACC"].ToString() + ", " + dtSource.Rows[0]["BANK"].ToString() + " ," + dtSource.Rows[0]["BRANCH"].ToString() + "[" + routingNo + "]";
                }

            }

            if (!(dtSource.Rows[looper]["WAR_RECEIVED_BY"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
                    WarDetails = "  Delivery to: " + dtSource.Rows[looper]["WAR_RECEIVED_BY"].ToString().ToUpper();
                }
                else
                {
                    WarDetails = WarDetails + " ; Delivery to: " + dtSource.Rows[looper]["WAR_RECEIVED_BY"].ToString().ToUpper();
                }
            }


            if (!(dtSource.Rows[looper]["WAR_DELEVERY_DATE"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
                    WarDetails = "Delivery  Date: " + Convert.ToDateTime(dtSource.Rows[looper]["WAR_DELEVERY_DATE"]).ToString("dd-MMM-yyyy");
                }
                else
                {
                    WarDetails = WarDetails + " ;Delivery Date: " + Convert.ToDateTime(dtSource.Rows[looper]["WAR_DELEVERY_DATE"]).ToString("dd-MMM-yyyy");
                }
            }

            if (!(dtSource.Rows[looper]["BEFTN_RETURN_DATE"].Equals(DBNull.Value)))
            {
                if (WarDetails == "")
                {
                    WarDetails = " BEFTN Return Date: " + Convert.ToDateTime(dtSource.Rows[looper]["BEFTN_RETURN_DATE"]).ToString("dd-MMM-yyyy");
                }
                else
                {
                    WarDetails = WarDetails + " ; BEFTN Return Date: " + Convert.ToDateTime(dtSource.Rows[looper]["BEFTN_RETURN_DATE"]).ToString("dd-MMM-yyyy");
                }
        ...
        public DataTable GetWarrantInfo(string filter)
        {
            ...
            sbMst.Append("SELECT INVEST.FUND.*, DIVIDEND.*, DIVIDEND_PARA.* FROM DIVIDEND, INVEST.FUND, DIVIDEND_PARA WHERE DIVIDEND.FUND_CODE = INVEST.FUND.F_CD ");
            sbMst.Append(" AND DIVIDEND.FUND_CODE = DIVIDEND_PARA.FUND_CODE AND DIVIDEND.DIVI_NO = DIVIDEND_PARA.DIVI_NO AND DIVIDEND.FY = DIVIDEND_PARA.FY AND  DIVIDEND.RECORD_DATE = DIVIDEND_PARA.RECORD_DATE ");
