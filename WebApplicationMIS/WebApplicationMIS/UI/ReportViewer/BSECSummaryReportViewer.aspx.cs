...
{
    public partial class BSECSummaryReportViewer : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (transtype == "C")
            {
                strSQLForPurchase = "SELECT  t.F_CD f_cd1, f.f_name fund_name1,SUM(AMT_AFT_COM) as COST1, sum(no_share) as No_Of_Share1 FROM INVEST.FUND_TRANS_HB  t, INVEST.fund f " +
            ...
            {
                strSQLForIPO = "SELECT  t.F_CD f_cd1, f.f_name fund_name1,SUM(AMT_AFT_COM)  as IPO, sum(no_share) as No_Of_Share1 FROM INVEST.FUND_TRANS_HB  t, INVEST.fund f " +
            ...
            else if (transtype == "R")
            {
                ...
                strSQLForRight = "SELECT  t.F_CD f_cd1, f.f_name fund_name1,SUM(AMT_AFT_COM) as Right, sum(no_share) as No_Of_Share1 FROM INVEST.FUND_TRANS_HB  t, INVEST.fund f " +
            ...
            else if (transtype == "S")
            {
                ...
                strSQLForSale = "SELECT t.F_CD f_cd2, f.f_name fund_name2,SUM(AMT_AFT_COM) as SALE2," +
                  "sum(no_share) as No_Of_Share2,sum(crt_aft_com * no_share) as COST2,SUM(AMT_AFT_COM) - sum(crt_aft_com * no_share) as profit2" +
                   " FROM INVEST.FUND_TRANS_HB t, INVEST.fund f WHERE t.TRAN_TP = 'S' AND VCH_DT BETWEEN '" + strFromdate + "' AND '" + strTodate + "' and t.F_CD IN(" + fundCodes + ") and f.f_cd = t.f_cd and t.f_cd <> 3" +
                    " GROUP BY t.F_CD, f.f_name ORDER BY t.F_CD, f.f_name";
