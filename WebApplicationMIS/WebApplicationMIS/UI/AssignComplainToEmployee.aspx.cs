...
{
    public partial class AssignComplainToEmployee1 : System.Web.UI.Page
    {
        ...
        public static string AssignComplainToEmployee(string Complainesistationmdl)
        {
            string msg = "";

            string strASSIGN_DATE;
            DateTime? dtstrASSIGN_DATE;
           // string id = "";
            ComplainResisterFunction complainResisterfunction = new ComplainResisterFunction();
            ...
            foreach (CompalinAction ComplainResiter in mdl)
            {
                ...
                if (!string.IsNullOrEmpty(ComplainResiter.ASSIGN_DATE))
                {
                    // dtcomplainDate = Convert.ToDateTime(serviceResiter.ComplainDate);
                    DateTime date = DateTime.ParseExact(ComplainResiter.ASSIGN_DATE, "dd/MM/yyyy", CultureInfo.InvariantCulture);

                    strASSIGN_DATE = date.ToString("dd-MMM-yyyy");
                }
                else
                {
                    dtstrASSIGN_DATE = null;
                    strASSIGN_DATE = "";
                }
                ...
                htcomplainResister.Add("STATUS", ComplainResiter.STATUS);
                htcomplainResister.Add("COMMENTS", ComplainResiter.COMMENTS.ToString());
                htcomplainResister.Add("ASSIGN_DATE", strASSIGN_DATE);
                htcomplainResister.Add("EMP_ID", ComplainResiter.EMP_ID.ToString());



                if (!string.IsNullOrEmpty(userName))
                {
                    htcomplainResister.Add("ASSIGN_BY", userName.ToString());
                }
            }
            ...
            foreach (var item in mdl)
            {
                ...
                dtcomplainResisterlist = complainResisterfunction.Get_DuplicateresiterID(item.COMPLAIN_ACTION_ID);

            }


            complainResisterfunction.SaveCOMPLAIN_ACTION(htcomplainResister);
            msg = "Assign Sucessfully";
